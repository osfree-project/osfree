...
extern int    gbm_readbuf_ahead(AHEAD *ahead, byte * buf, int buflen);

typedef void WCACHE;
extern WCACHE *gbm_create_wcache  (int fd);
extern int     gbm_destroy_wcache (WCACHE *wcache);
extern int     gbm_write_wcache   (WCACHE *wcache, byte c);
extern int     gbm_writebuf_wcache(WCACHE *wcache, const byte * buf, int buflen);
