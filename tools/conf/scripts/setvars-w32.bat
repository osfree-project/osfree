...
set SVN_SSL_CLIENT_PROTOCOL=TLSv1.1 

set osfreebe=%root%osFreeBE
set mingit=%root%tools\conf\mingit\cmd
set svnc=%root%tools\conf\svnportable\svn
...
set PATH=%mingit%;%svnc%;%OS2TK%\bin;%OS2TK%\som\bin;%OS2TK%\som\common\dll;%WATCOM%\binnt;%WATCOM%\binw;%TOOLS%;%FPPATH%;%svn%;%regina%;\data\dev\qemu;\data\dev\cdrtools;\data\dev\Bochs-2.3.5;\data\dev\bin;\usr\local\wbin;%PATH%;
