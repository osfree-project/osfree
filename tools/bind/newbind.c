...
void generate_imptable(void)
{
    ...
    while (current)
    {
        ...
        *curbuf=strlen(current->func);
        curbuf++;
        strcpy((char*)curbuf,current->func);
    ...
    fwrite(buf, 1, (curbuf-(BYTE*)&buf)+1, f);
    ...
    {
        // copy modname
        strcpy((char*)&buf[*((WORD *)&buf[1])+2],current->mod);
        // copy funcname
        strcpy((char*)&buf[*((WORD *)&buf[1])+2+9],current->func);
                                                                    ...
                                                                    if (((rlc.nr_flags & NRRTYP)==NRRORD) || ((rlc.nr_flags & NRRTYP)==NRRNAM))
                                                                    {
                                                                        if ((rlc.nr_flags & NRRTYP)==NRRNAM)
                                                                        {
                                                                            ...
                                                                            if (fname) {
                                                                                ...
                                                                                // FIXED: Исправлено сравнение имен функций
