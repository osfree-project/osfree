...
begin
  // Имя константы следует сразу после ключевого слова constant
  Result := TPasConst(CreateElement(TPasConst, ExpectIdentifier, Parent));
  try
    NextToken;
...
begin
  TypeName := ExpectIdentifier;   // Имя типа читается здесь
  ExpectToken(tkEqual);
  NextToken;
  case CurToken of
    tkClass:
...
begin
  // Имя переменной следует сразу после ключевого слова variable
  Result := TPasVariable(CreateElement(TPasVariable, ExpectIdentifier, Parent));
  NextToken;
  ExpectToken(tkColon);
  Result.VarType := ParseType(nil);
  ExpectToken(tkSemicolon);
