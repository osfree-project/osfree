...
nearest_power (unsigned long base, unsigned long num)
{
  if (num > MY_MAXSIZE / 2)
    {
      return MY_MAXSIZE;
 ...
 */
GStringChunk*
g_string_chunk_new (unsigned long size)
{
  GStringChunk *new_chunk = ((GStringChunk *) malloc (((unsigned long) sizeof (GStringChunk)) * ((unsigned long) (1))));

  unsigned long actual_size = 1;
...
//  g_return_val_if_fail (chunk != NULL, NULL);
  ...
  lookup = (char*) g_hash_table_lookup (chunk->const_table, (char *)string);
...
//  g_return_val_if_fail (chunk != NULL, NULL);
  ...
  if ((chunk->storage_next + size + 1) > chunk->this_size)
    {
      unsigned long new_size = nearest_power (chunk->default_size, size + 1);

      chunk->storage_list = g_slist_prepend (chunk->storage_list,
                                             (char *) malloc(sizeof(char)*new_size));
 ...
 */
GString*
g_string_sized_new (unsigned long dfl_size)
{
...
//    g_return_val_if_fail (pos <= string->len, string);
  ...
  if (val >= string->str && val <= string->str + string->len)
    {
      unsigned long offset = val - string->str;
