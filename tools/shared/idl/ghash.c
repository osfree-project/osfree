                          ...
                          unsigned int         *hash_return)
{
  ...
  printf("table_lookup_node 1\n");

  hash_value = (* hash_table->hash_func) (key);
  node_ptr = &hash_table->nodes[hash_value % hash_table->size];

  printf("table_lookup_node 2\n");

  if (hash_return)
    *hash_return = hash_value;

  printf("table_lookup_node 3\n");
  ...
  printf("table_lookup_node 5\n");

          node_ptr = &(*node_ptr)->next;
        }
    }
  printf("table_lookup_node 6\n");
...
g_hash_table_resize (GHashTable *hash_table)
{
  ...
  new_nodes = ((GHashNode* *) malloc (((unsigned int) sizeof (GHashNode*)) * ((unsigned int) (new_size))));
                       ...
                       GDestroyNotify  value_destroy_func)
{
  ...
  hash_table->nodes              = ((GHashNode* *) malloc (((unsigned int) sizeof (GHashNode*)) * ((unsigned int) (hash_table->size))));
...
//  g_return_if_fail (hash_table->ref_count > 0);
  ...
  printf("table_insert_internal 1\n");
  node_ptr = g_hash_table_lookup_node (hash_table, key, &key_hash);

  printf("table_insert_internal 2\n");
  ...
  printf("table_insert_internal 3\n");
  ...
  printf("table_insert_internal 4\n");
