...
void sha1_update(SHA1_CTX *ctx, const unsigned char *data, size_t len) {
    ...
    /* Считаем биты как (len * 8), разбитое на два 32-битных слова.
     * Для len < 2^29 (что всегда верно для реальных блоков чтения)
     * результат корректен на любой платформе. */
    bits_lo = (unsigned int)len << 3;
    bits_hi = (unsigned int)(len >> 29);

    index = (ctx->count[0] >> 3) & 0x3F;
    prev = ctx->count[0];
    ctx->count[0] += bits_lo;
    if (ctx->count[0] < prev) ctx->count[1]++;
    ctx->count[1] += bits_hi;
