...
/* Экранирует строку для вставки в JSON-значение (внутри двойных
 * кавычек) согласно RFC 8259 §7:
 *   - quotation mark ("), reverse solidus (\) — MUST escape;
 *   - control characters U+0000..U+001F — MUST escape;
 *   - короткие формы \b, \f, \n, \r, \t — для U+0008, U+000C, U+000A,
 *     U+000D, U+0009 соответственно;
 *   - остальные управляющие (U+0000..U+0007, U+000B, U+000E..U+001F) —
 *     через \u00XX.
...
char *json_escape_string(const char *src) {
    ...
    /* Точный расчёт размера результата. */
    size = 0;
    for (i = 0; i < len; i++) {
