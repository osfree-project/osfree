 ...
 * keeps the most specific diagnostic when the parser backtracks.
 *
...
static PSZ parse_literal_string(PARSE *pParser) {
    ...
    {
        size_t cbLen = (size_t)(pParser->pszPos - pszStart);
        PSZ pszResult = (PSZ)malloc(cbLen + 1);
        if (!pszResult) return NULL;
        memcpy(pszResult, pszStart, cbLen);
        pszResult[cbLen] = '\0';
        pParser->pszPos++;
        return pszResult;
    }
...
static PSZ parse_multiline_literal_string(PARSE *pParser) {
    ...
    if (!pszContentEnd) return NULL;
    {
        ...
        cbLen = (size_t)(pszContentEnd - pszStart);
...
static int parse_number_or_datetime(PARSE *pParser, PTOMLVALUE *ppValue) {
    ...
    if (looks_like_datetime(pParser->pszPos, pParser->pszEnd)) {
        ...
        pszTok = (PSZ)malloc(cbTokLen + 1);
        if (!pszTok) return -1;
        memcpy(pszTok, pszTokStart, cbTokLen);
        pszTok[cbTokLen] = '\0';
...
static PSZ parse_key_part(PARSE *pParser) {
    ...
    if (*pParser->pszPos == '\'') return parse_literal_string(pParser);
    {
        ...
        cbLen = (size_t)(pParser->pszPos - pszStart);
        pszKey = (PSZ)malloc(cbLen + 1);
        if (!pszKey) return NULL;
        memcpy(pszKey, pszStart, cbLen);
        pszKey[cbLen] = '\0';
        return pszKey;
    }
}
 ...
 * buffers and returns a document handle. The module owns the
 * internal structures and releases them in TomlClose.
 *
 * @param[in]  pszPath  Path to the file. Not NULL.
 ...
 * and node handles associated with the document. After return the
 * handle is invalid.
 *
 * @param[in] hToml  Document handle. NULLHANDLE is accepted and
 *                   treated as a no-op.
 *
 * @return APIRET
 ...
 *
 * @warning Calling TomlClose twice with the same handle is
 ...
 * NUL. If the buffer is too small, returns ERROR_BUFFER_OVERFLOW
 * and writes the required size (including NUL) to *pulSize.
 *
 ...
 * to obtain the root node. Handles remain valid until TomlClose and
 * do not need to be released individually.
 *
 * @param[in]  hToml    Handle. Not NULLHANDLE.
 ...
 *                                      table.
 *
 * @note The cursor owns internal buffers allocated from the
 *       document. Always release it via TomlFindClose.
 ...
 * @retval ERROR_INVALID_PARAMETER  Any parameter is NULL.
 * @retval ERROR_INVALID_HANDLE     Handle is not recognized.
 * @retval ERROR_FILE_NOT_FOUND     Cursor has no current key.
 * @retval ERROR_BUFFER_OVERFLOW    Buffer too small.
 */
 ...
 * @retval ERROR_INVALID_PARAMETER      Any parameter is NULL.
 * @retval ERROR_INVALID_HANDLE         Handle is not recognized.
 * @retval ERROR_FILE_NOT_FOUND         Cursor has no current value.
 * @retval ERROR_BUFFER_OVERFLOW        Buffer too small.
