...
mmap_avail_at (unsigned long bottom)
{
  ...
  do
    {
      for (cont = 0, addr0 = mbi.mmap_addr, addr = (void far *) FP_FROM_PHYS(addr0);
           addr0 < mbi.mmap_addr + mbi.mmap_length;
           addr0 += *((unsigned long far *) addr) + 4, addr = (void far *) FP_FROM_PHYS(addr0))
        {
          struct AddrRangeDesc far *desc = (struct AddrRangeDesc far *) addr;
...
{
#ifndef STAGE1_5
  unsigned long cont, memtmp, addr0;
  void far *addr;
  //int drive;
  long drive;
#endif
  ...
  printk("(1) mbi.mem_lower = %u", mbi.mem_lower);
  printk("(1) mbi.mem_upper = %u", mbi.mem_upper);
  ...
  do
    {
      cont = get_mmap_entry (addr0, cont);

      addr = (void far *) FP_FROM_PHYS(addr0);

      /* If the returned buffer's length is zero, quit. */
      if (! *((unsigned long far *) addr))
        break;

      mbi.mmap_length += *((unsigned long far *) addr) + 4;
      printk("addr = 0x%04x:0x%04x", FP_SEG(addr), FP_OFF(addr));
      printk("addr0 = 0x%08lx, cont = %ld, len = %lu", addr0, cont,
              *((unsigned long far *) addr) );

      addr0 += *((unsigned long far *) addr) + 4;
    }
  while (cont);
  ...
  if (mbi.mmap_length)
    {
       ...
       */
      printk("(2) mbi.mem_lower = %u", mbi.mem_lower);
      printk("(2) mbi.mem_upper = %u", mbi.mem_upper);

      mbi.mem_lower = mmap_avail_at (0) >> 10;
      mbi.mem_upper = mmap_avail_at (0x100000) >> 10;

      printk("(3) mbi.mem_lower = %u", mbi.mem_lower);
      printk("(3) mbi.mem_upper = %u", mbi.mem_upper);

      /* Find the maximum available address. Ignore any memory holes.  */
      for (max_addr = 0, addr0 = mbi.mmap_addr, addr = (void far *) FP_FROM_PHYS(addr0);
           addr0 < mbi.mmap_addr + mbi.mmap_length;
           addr0 += *((unsigned long far *) addr) + 4, addr = (void far *) FP_FROM_PHYS(addr0))
        {
          struct AddrRangeDesc far *desc = (struct AddrRangeDesc far *) addr;
  ...
  for (drive = 0x80; drive < 0x88; drive++)
    {
      ...
      struct drive_info far *info = (struct drive_info far *) addr;
      ...
      addr0 += sizeof (struct drive_info);
      addr = (void far *) FP_FROM_PHYS(addr0);
      for (port = io_map; *port; port++, addr0 += sizeof (unsigned short),
           addr = (void far *) FP_FROM_PHYS(addr0))
  ...
  mbi.boot_loader_name = (unsigned long) "FreeLdr";

  /* Get the APM BIOS table.  */
  get_apm_info ();
  if (apm_bios_info.version)
    mbi.apm_table = (unsigned long) PHYS_FROM_NP(current_seg, &apm_bios_info);
