...
register_module(char * name, void * mod_struct) {
	...
	struct module_rec * new_mod = (struct module_rec *) malloc(sizeof(struct module_rec));
	new_mod->mod_name = (char *)malloc(strlen(name)+1);
	strcpy(new_mod->mod_name, name);
	struct module_rec * prev = &module_root;
...
void * find_module(char * name, struct t_processlx *proc) {
	...
	void *ptr_mod = load_module(name, proc);
...
void find_module_path(char * name, char * full_path_name) {
	...
	char * lcase_name = (char*)&buf_lcase_name;
	int i =0;
	FILE *f=0;
	do {
...
void * load_module(char * name, struct t_processlx *proc) {
	...
	FILE *f = fopen(p_buf, "rb");  /* Open file in read only binary mode, in case this code 
	...
	if(f && load_lx(f, lx_exe_mod)) {     /* Load LX header.*/
		...
		struct module_rec * new_module_el = register_module(name, lx_exe_mod);		
