...
//         enter_kdebug("l4rm_area_setup_region failed");
    /* open new ds 2 */
    ret = l4dm_mem_open(dm_id,8192,0,0,"l4rm_test2",&ds2);
    if (ret)
    {
    ...
    ret=
    l4dm_map_pages(&ds2, 0, 8192,     // 0 - 8192 i ds                   
                         0, 13 /*2^13=8192*/, 0,  // 0 - 8192 i rcv window, offset=0 
...
void * l4_alloc_mem(int base, int size, int flags) {
 ...
 /* l4rm_attach(&ds,              size,      base,      l4_flags,    &addr); */
    int st = l4rm_attach_to_region(&ds,    base,      size,   0,         l4_flags);
    addr = base;
