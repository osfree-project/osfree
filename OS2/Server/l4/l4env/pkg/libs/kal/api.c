...
attach_ds(l4dm_dataspace_t *ds, l4_uint32_t flags, l4_addr_t *addr)
{
  ...
  if ((error = l4dm_mem_size(ds, &size)))
    {
      LOG_printf("Error %d (%s) getting size of dataspace\n",
  ...
  if ((error = l4rm_attach(ds, size, 0, flags, addr)))
    {
      LOG_printf("Error %d (%s) attaching dataspace\n",
	  error, l4env_errstr(error));
      return error;
    }
  return 0;
...
{
  CORBA_Environment env = dice_default_environment;
  l4dm_dataspace_t ds_t, ds_p;
  l4_addr_t addr_t, addr_p;
  APIRET rc;

  STKIN
  /* get the dataspace with info blocks */
  rc = os2server_dos_GetTIB_call (&os2srv, &ds_t, &env);
  ...
  /* attach it */ 
  rc = attach_ds(&ds_t, L4DM_RW, &addr_t);
  if (rc)
  {
    LOG("error attaching ds_t!");
    STKOUT
    return rc;
  }
  else
  ...
  *pptib = (PTIB)((char *)addr_t);

  /* get the dataspace with info blocks */
  rc = os2server_dos_GetPIB_call (&os2srv, &ds_p, &env);
  ...
  /* attach it */ 
  rc = attach_ds(&ds_p, L4DM_RW, &addr_p);
  if (rc)
  {
    LOG("error attaching ds_p!");
    STKOUT
    return rc;
  }
  else
  ...
  *pppib = (PPIB)((char *)addr_p);

  (*pptib)->tib_ptib2 = (PTIB2)((char *)(*pptib)->tib_ptib2 + (unsigned)addr_t);
  
  /* fixup fields */
  (*pppib)->pib_pchcmd = (char *)((*pppib)->pib_pchcmd) + (unsigned)addr_p;
  (*pppib)->pib_pchenv = (char *)((*pppib)->pib_pchenv) + (unsigned)addr_p;

  LOG("arg=0x%x", (*pppib)->pib_pchcmd);
  LOG("env=0x%x", (*pppib)->pib_pchenv);

  /* detach dataspace */
  rc = l4rm_detach(addr_t);
  ...
  /* then attach them readonly */
  rc = attach_ds_reg(ds_t, L4DM_RO, addr_t);
  ...
  rc = attach_ds_reg(ds_p, L4DM_RO, addr_p);
