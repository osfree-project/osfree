 ...
 * 20 Jul 00 - Mike: Prevent saving wrong "main" symbol; changed GetMainEntry-
 *              Point() to work of the extern table entry for main,
 *              and follow the expected BL instruction there
 * 27 Apr 01 - Mike: Use the symbol table main, rather than the entry point
 *              main. No need to follow B.L instructions. Also save STUB symbol
 *              entries (e.g. library functions)
 * 02 May 01 - Mike: Fixed m_iNumSections (was not assigned); fixed the size of
 *              the first section (header)
 * 03 May 01 - Mike: Set size of first section to 0 (else GetSectionInfoByAddr
 *              may return section zero when header is large enough)
 * 09 May 01 - Mike: Read the imports table so can identify library functions
 * 14 May 01 - Mike: Implemented (slow) GetAddressByName
 * 15 May 01 - Mike: Fixed an endianness problem reading the import table
 * 14 Jun 01 - Mike: Removed "main at <address> has type ..." diagnostic
 * 24 Jul 01 - Mike: Allow symbols of type 12 (millicode) into symbol table;
 *              code to guess start of main when no symbols at all
 * 30 Jul 01 - Mike: Implement GetGlobalPointerInfo
 * 01 Aug 01 - Mike: Changed GetGlobalPointerInfo to the new definition
 * 03 Aug 01 - Mike: Many changes to use the "dynamic" symbols in the import
 *              table; essential when an input binary is stripped
 * 09 Aug 01 - Mike: Just cleaned up some warnings, etc
 * 14 Aug 01 - Mike: in GetDynamicGlobalMap(), ignore names of -1
 * 17 Aug 01 - Mike: Changed tSyms to be map from string to address, to avoid
...
bool HpSomBinaryFile::RealLoad(const char* sName)
{
    ...
    const export_entry* export_list = (export_entry*)(DLTable +
      UINT4(DLTable + 0x20));
...
//  m_pSections[0].uSectionSize = AUXHDR(4);
    ...
    pair<unsigned, int> pr = getSubspaceInfo("$GLOBAL$");
...
//cout << "Importing " << (pDlStrings+import_list[u].name) << endl;
        tSyms[pDlStrings + import_list[u].name] = PLTs[v].value;
...
//cout << "Exporting " << (pDlStrings+export_list[u].name) << " value " << hex << export_list[u].value << endl;
        if (strncmp(pDlStrings+export_list[u].name, "main", 4) == 0) {
            // Enter the symbol "_callmain" for this address
            tSyms["_callmain"] = export_list[u].value;
...
//  cout << "main at " << hex << value << " has type " << SYMBOLTY(u) << endl;}
            ...
            // is correct).
            map<const char*, ADDRESS>::iterator it = tSyms.find(pSymName);
            if (it == tSyms.end())
...
//cout << "Symbol " << pNames+SYMBOLNM(u) << ", type " << SYMBOLTY(u) << ", value " << hex << value << ", aux " << SYMBOLAUX(u) << endl;  // HACK!
    ...
    // Enter the values
    map<const char*, ADDRESS>::const_iterator it;
...
list<SectionInfo*>& HpSomBinaryFile::GetEntryPoints(const char* pEntry = "main")
{
...
list<const char *> HpSomBinaryFile::getDependencyList()
{
    return list<const char *>(); /* FIXME */
}
...
const char* HpSomBinaryFile::SymbolByAddress(const ADDRESS dwAddr)
{
    return symbols.Find(dwAddr);
}

ADDRESS HpSomBinaryFile::GetAddressByName(const char* pName,
...
pair<ADDRESS, int> HpSomBinaryFile::getSubspaceInfo(const char* ssname)
{
    pair<ADDRESS, int> ret(0, 0);
    ...
    for (unsigned u=0; u < numSubSpaces; u++) {
        char* thisName = (char*)(spaceStrings + subSpaces[u].name);
        unsigned thisNameSize = UINT4(spaceStrings + subSpaces[u].name - 4);
//cout << "Subspace " << thisName << " starts " << hex << subSpaces[u].subspace_start << " length " << subSpaces[u].subspace_length << endl;
        if ((thisNameSize == strlen(ssname)) &&
          ((strcmp(thisName, ssname) == 0))) {
            ret.first = subSpaces[u].subspace_start;
            ret.second = subSpaces[u].subspace_length;
            return ret;
        }
    }
...
pair<unsigned,unsigned> HpSomBinaryFile::GetGlobalPointerInfo()
{
    pair<unsigned, unsigned> ret(0, 0);
    // Search the subspace names for "$GLOBAL$
    pair<ADDRESS, int> info = getSubspaceInfo("$GLOBAL$");
 ...
 *============================================================================*/
map<ADDRESS, const char*>* HpSomBinaryFile::GetDynamicGlobalMap()
{
    // Find the DL Table, if it exists
    // The DL table (Dynamic Link info) is supposed to be at the start of
    // the $TEXT$ space, but the only way I can presently find that is to
    ...
    map<ADDRESS, const char*>* ret = new map<ADDRESS, const char*>;
