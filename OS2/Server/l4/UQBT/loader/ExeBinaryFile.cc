...
/* EXE binary file format.
	...
	MVE 08/10/97
 * 16 Mar 01 - Mike: Added appropriate empty functions for the mandatory
 *                virtuals like getImageBase(), Open(), etc
 * 17 Mar 01 - Mike: Host addresses start at 0 now
 * 18 Mar 01 - Mike: Removed 0x100 "PSP" bytes from start of image
 * 31 Mar 01 - Mike: SymbolByAddr recognises main
*/
...
list<const char *> ExeBinaryFile::getDependencyList()
{
    return list<const char *>(); /* for now */
}
...
list<SectionInfo*>& ExeBinaryFile::GetEntryPoints(const char* pEntry = "main")
{
