                            ...
                            unsigned long * phmod)
{
  ...
  *phmod=0;
  ...
  {
    if (options.debugmodmgr) io_printf("ModLoadModule: EXMWRAP module replaced by SUB32 module.\n");
    pszModname="SUB32";
  }

  if (options.debugmodmgr) io_printf("ModLoadModule: Loading module %s...\n", pszModname);
  ...
  while(prev)
  {
    if(strcasecmp(pszModname, prev->mod_name)==0)
    {
      ...
      {
        *phmod=NULL;
        if (cbName<=strlen(pszModname)) return 8 /*ERROR_NOT_ENOUGH_MEMORY*/;
