                           ...
                           CORBA_Server_Environment *_dice_corba_env)
{
  ...
  if (handle == -1)
  {
    if (errno == EMFILE)
    {
      LOG("ERROR_TOO_MANY_OPEN_FILES");
      return 4; /* ERROR_TOO_MANY_OPEN_FILES */
    }
      
    if (errno == ENOENT)
    {
      LOG("ERROR_FILE_NOT_FOUND");
                                   ...
                                   CORBA_Server_Environment *_dice_corba_env)
{
	...
	{
          rc = os2server_dos_QueryCurrentDisk_call(&os2srv, &ulDrvNum, &ulDrvMap, &env);
