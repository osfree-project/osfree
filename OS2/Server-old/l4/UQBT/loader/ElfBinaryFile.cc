 ...
 *  MVE 30/9/97
 *  MVE 7/11/97 Added symbol table as a SymTab object
 *  MVE 11/12/97 Major mods, e.g. removing pseudo-headers
 *  MVE 24/12/97 Fixed a problem with m_pReloc
 * 3 Mar 98 - Cristina
 *  changed ADDR for ADDRESS for consistency with other tools.
 * 11 Mar 98 - Cristina 
 *  replaced BOOL for bool type (C++'s), same for TRUE and FALSE. 
 * 18 Mar 98 - Mike
 *      Simplified IsDynamicLinkedProc(); now store native addresses
 *      in m_uPltMin and m_uPlyMax
 * 27 Mar 98 - Cristina
 *  added GetMainEntryPoint().
 * 29 May 98 - Mike
 *  added SearchValueByName() to linearly search .symtab sections for
 *  files without .hash sections (e.g. object files)
 * 6th Aug 98 - Mike
 ...
 *  When search for "main" entry point, allow STT_NOTYPE; also add main
 *  to m_Sym if has STT_NOTYPE
 * 20 Oct 98 - Mike
 *  GetEntryPoints() now takes an optional const char* for an optional
 *  symbol to replace main as the starting function
 * 12 Dec 98 - Mike: augmented ValueByName to do a linear search of the main
 *  symbol table if a search of the hash table fails. Finds init_dummy now
 * 05 Mar 99 - Mike: SectionInfo.bData set now (why not earlier?)
 * 20 Apr 99 - Mike: Added GetDistanceByName() (for coverage analysis)
 * 25 Aug 99 - Mike: Changed functions GetDistanceByName() and
 *              SearchValueByName() to operate on .symtab and .dynsym
 * 14 Dec 99 - Mike: bBss set more sensibly now (based on name of section)
 * 05 Jul 00 - Mike: .got is considered a data section now
 * 02 Aug 00 - Mike: Added GetImportStubs()
 * 12 Mar 01 - Mike: No #include of "sys/link.h" if LINUX
 * 10 Apr 01 - Mike: GetImportStubs uses return value from FindNext correctly.
 *              Prevents infinite loop and/or segfault at end of translation
 * 12 Apr 01 - Mike: Fixed an invalid use of str(); Linux makes this an error
 * 17 Apr 01 - Mike: Fixed DisplayDetails to compile on Linux
 * 12 Sep 01 - Mike: Replaced SymTab table, which could only handle one of the
 *              ELF symbol tables (main or dynamic) with a map from ADDRESS
 *              to string (with symbols from both tables)
 * 17 Oct 01 - Mike: Remove the @@GLIBC_2.0 from Linux symbols
 * 23 Oct 01 - Mike: Fixed a problem with UQBT's -C, caused ultimately by not
 *              being strict enough about what is and is not a bData section
*/
...
void ElfBinaryFile::AddSyms(const char* sSymSect, const char* sStrSect)
{
    ...
    for (int i = 1; i < nSyms; i++)
    {
        if (ELF32_ST_TYPE(m_pSym[i].st_info) == STT_FUNC) {
            ...
            string str(GetStrPtr(idx, m_pSym[i].st_name));
            // Hack of the "@@GLIBC_2.0" of Linux, if present
            unsigned pos;
            if ((pos = str.find("@@")) != string::npos)
                str.erase(pos);
            map<ADDRESS, string>::iterator aa = m_SymA.find(val);
...
{
    map<ADDRESS, string>::iterator aa = m_SymA.find(dwAddr);
    if (aa == m_SymA.end())
        return 0;
...
//
list<SectionInfo*>& ElfBinaryFile::GetEntryPoints(
...
list<const char *> ElfBinaryFile::getDependencyList()
{
...
ADDRESS* ElfBinaryFile::GetImportStubs(int& numImports)
{
    ADDRESS a = m_uPltMin;
    int n = 0;
    map<ADDRESS, string>::iterator aa = m_SymA.find(a);
    map<ADDRESS, string>::iterator ff = aa;
    bool delDummy = false;
    if (aa == m_SymA.end()) {
...
void ElfBinaryFile::writeObjectFile
    (string &path, const char* name, void *ptxt, int txtsz, RelocMap& reloc)
{
...
// So far, only dealing with Sparc binaries -- FIXME!
     ...
     */
    string Name = path + name + string(".o");
...
//  int rela_text_idx = ++next_idx;     // Relocation table
    ...
    sdata->d_buf = ptxt;
    sdata->d_size = txtsz;
    sdata->d_align = TGTINSTSIZE;   // Instr alignment
    ...
    shdr->sh_addralign = TGTINSTSIZE;  // Instr alignment
 ...
 *============================================================================*/
map<ADDRESS, const char*>* ElfBinaryFile::GetDynamicGlobalMap()
{
