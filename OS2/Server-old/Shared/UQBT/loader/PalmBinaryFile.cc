...
list<SectionInfo*>& PalmBinaryFile::GetEntryPoints(const char* pEntry = "main")
{
...
list<const char *> PalmBinaryFile::getDependencyList()
{
    return list<const char *>(); /* doesn't really exist on palm */
}
...
pair<unsigned,unsigned> PalmBinaryFile::GetGlobalPointerInfo()
{
    unsigned agp = 0;
    const SectionInfo* ps = GetSectionInfoByName("data0");
    if (ps) agp = ps->uNativeAddr;
    pair<unsigned, unsigned> ret(agp, m_SizeBelowA5);
    return ret;
}
...
void PalmBinaryFile::GenerateBinFiles(const string& path) const
{
    for (int i=0; i < m_iNumSections; i++) {
