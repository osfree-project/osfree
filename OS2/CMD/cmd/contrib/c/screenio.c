...
static int nColor;
int fInsert;
int fLead;                              // DBCS lead byte flag
int fCtrlUpDown;                        // Ctrl up/down key entered ?
...
int egets( char *pStr, int nMaxLength, int fFlags)
{
    ...
    for ( ; ; ) {
        ...
        if ( fCtrlUpDown && c != CTL_DOWN && c != CTL_UP ) {    // switch to previous cursor
            gpIniptr->CursO = iCurOver;
            gpIniptr->CursI = iCurIns;
            SetCurSize( curstate );
        ...
        switch ( c ) {
            ...
            case CTL_PgDn:
                {
                    ...
                    if ( gnPopExitKey == CR ) {
                        ...
                        if ( saved_char != PgUp ) {
                            // Suppress fuzzy since no wildcards should exist in history list 09 May 10 SHL
                            __cd( source, CD_CHANGEDRIVE | CD_SAVELASTDIR | CD_NOFUZZY );
                            if ( *base == '\0' )
                                goto egots_cr_key;
                        } else
                            goto egots_cr_key;
            ...
            case CTL_UP:
                TRACE("CTRL-up detected, nCursorRow=%d", nCursorRow);
                if ( !fCtrlUpDown ) {   // switch to block cursor
                    fCtrlUpDown = TRUE;
                    iCurOver = gpIniptr->CursO;
                    gpIniptr->CursO = -100;
                    iCurIns = gpIniptr->CursI;
                    gpIniptr->CursI = -2;
                    iTempCurRow = nCursorRow - nScreenRows / 8;     // first time make a bigger jump
                    if (iTempCurRow < 0) iTempCurRow = 0;
                    SetCurSize( curstate );
                }
                iTempCurRow -= nScreenRows / 6;
                if (iTempCurRow < 0) iTempCurRow = 0;
                SetCurPos( iTempCurRow, nCursorColumn);
                goto egets_key;
                break;
            case CTL_DOWN:
                TRACE("CTRL-down detected, nCursorRow=%d", nCursorRow);
                if ( !fCtrlUpDown ) {   // switch to block cursor
                    fCtrlUpDown = TRUE;
                    iCurOver = gpIniptr->CursO;
                    gpIniptr->CursO = -100;
                    iCurIns = gpIniptr->CursI;
                    gpIniptr->CursI = -2;
                    iTempCurRow = nCursorRow;
                    SetCurSize( curstate );
                }
                iTempCurRow += nScreenRows / 6;
                if (iTempCurRow > nScreenRows) iTempCurRow = nScreenRows;
                SetCurPos( iTempCurRow, nCursorColumn);
                goto egets_key;
                break;
            ...
            case DEL:
                if ( c == BACKSPACE ) {
                    ...
                    if ( curpos == base )
                        break;
                    curpos = AdjustPos( curpos - 1 );

                }

                if ( *curpos ) {
                    c = PositionEGets( strend( curpos ));
                    strcpy( curpos, curpos + ( IsDBCSLead( *curpos ) ? 2 : 1 ));
                    c -= PositionEGets( strend( curpos ));
                    PositionEGets( curpos );
                    efputs( curpos );
                    PadBlanks( c );
            ...
            case INS:
                {
                // check for shift key and if down drop through to case 22
                KBDINFO kbdinfo;
                int rc;

                kbdinfo.cb = 10;
                rc = KbdGetStatus(&kbdinfo, 0);

                if (!(kbdinfo.fsState  & 3))
                    fInsert ^= 1;
                    curstate ^= 1;
                    SetCurPos( nCursorRow, nCursorColumn );
                    SetCurSize( curstate );
                ...
                // Drop through for Shift-INS

            case 22: // Ctrl-V : paste first clipboard line.
                {
                    static CHAR s_szClipboardLine[4096]; // assumes we don't get recursive calls.
                    unsigned long  ulState;
                    PCSZ pszSrc;

                    kptr = &s_szClipboardLine[0];
                    *kptr = '\0';

                    if ( GetClipboardText( &ulState, &pszSrc ) == 0 ) {

                        // copy the first line into the static buffer.
                        PCH pszDst = kptr = &s_szClipboardLine[0];
                        CHAR ch;
                        do {
                            ch = *pszSrc++;
                            if ( !ch )
                            ...
                            switch ( ch ) {
                                ...
                                default:
                                  ...
                                  && (pszDst - &s_szClipboardLine[0]) < sizeof( s_szClipboardLine ) - 2 );
                        *pszDst = '\0';
                    ...
                    if ( nEditFlag & EDIT_COMMAND ) {
                        ...
                        for ( offset = -1, fval = FIND_FIRST; ; ) {
                            ...
                            for ( n = 0; ; ) {
                                ...
                                ptr = skipspace( base );
                                ptr = ntharg( base, *ptr == '\'' ? 0 : NTHARG_BACKQUOTE_IS_DATA );
            ...
            default:
                ...
                // check for invalid (421 is ALT-TAB in a full screen session)
                if ( c > 0xFF  && c != 421)  {
                    honk();
                }
                if (c == 421) {
                ...
                // check for overlength entry
                else if ( ( strlen( base ) >= (unsigned int)nMaxLength ) && ((*curpos == '\0' ) || (fInsert)) ) {
                    if ( fLead ) {
                        fLead = 0;
                        curpos--;
                    ...
                    int fTrail;          // DBCS trail flag
                    int fBlank;          // blank to overwrite SBCS char on DBCS char

                    fTrail = fLead;
                    fLead = ( !fLead && IsDBCSLead( c )) ? 1 : 0;
                    ...
                    if ( fInsert )
                        ...
                        clearline( curpos );
                    else if ( IsDBCSLead( *curpos ) ) {
                        if ( !fLead && !fTrail ) {  // overwrite SBCS char on DBCS char
                            memmove( curpos, curpos + 1, strlen( curpos + 1 ) + 1 );
                            fBlank = 1;
                        }
                    } else if ( fLead ) {     // overwrite DBCS char on SBCS char
                        if ( strlen( base ) >= nMaxLength ) {
                            fLead = 0;
                            honk();
                            break;
                        }

                        memmove( curpos + 2, curpos + 1, strlen( curpos + 1 ) + 1 );
                    }
...
static int PositionEGets( char *pCur )
{
    ...
    // check for line wrap
    ptr = base;
    soc = ptr + ( nScreenColumns - nHomeColumn );
    for ( nCursorRow = nHomeRow; nCursorColumn + 1 >= nScreenColumns; nCursorRow++ ) {
        if ( IsDBCSTrailStr( ptr, soc ) ) {
            nCursorColumn++;
            soc--;
        }
        ...
        ptr = soc;
        soc += nScreenColumns;
        if ( nCursorColumn < nScreenColumns )
            break;
        nCursorColumn -= nScreenColumns;
    }
...
static char *AdjustPos( char *pos )
{
    return IsDBCSTrailStr( base, pos ) ? ( pos - 1 ) : pos;
}
