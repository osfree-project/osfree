...
{
    *row = *col = 0;
    (void)VioGetCurPos( (VIO_PUTYPE)row, (VIO_PUTYPE)col, 0 );
}
...
{
    CHAR achCells[2];
    VIO_UTYPE cb = sizeof(achCells);
    VIO_UTYPE row = 0, column = 0;
...
void SetCurSize( int c_type )
{
    ...
    // TRACE("SerCurSize, CursO=%d, CursI=%d", gpIniptr->CursO, gpIniptr->CursI );
