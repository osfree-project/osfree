...
static int IniFProc(char *fname, INIFILE *InitData, unsigned int fIgnoreSection)
{
    ...
    // 2016-10-23 SHL Avoid confusion if invoked as 4os2 /c @foo.exe
    // People write address cmd '@foo.exe' in VXREXX and other compiled REXX code
    nAppType = app_type( fname );
    if ( nAppType >= 0 ) {
        // Looks like an executable, not an INI file
        rval = -1;
        goto IFProcExit;
    }

    // 2016-10-23 SHL Avoid confusion if invoked as 4os2 /c @foo.cmd etc.
    if ( ( ptr = ext_part( fname ) ) != NULL ) {
      int i;
