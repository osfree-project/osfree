...
static int _fmtin(const char *, const char *, va_list);
...
static int _fmtin(const char *source, const char *fmt, va_list arg)
{
    const char *base, *iptr;
    char *optr;
...
next_fmt:
            ...
            case 's':
                ...
                if (ignore == 0)
                    optr = (char *)(va_arg(arg, char *));

                for ( ; ((width > 0) && (*source)); source++, width--) {

                    if (using_table) {
                        if (table[(unsigned)*source] == 0)
                            break;
            ...
            case 'x':
                ...
                if (*fmt == 'x') {
                    ...
                    for (iptr = source, value = 0; ((width > 0) && (isxdigit(*source))); source++, width--) {
                    ...
                    for (iptr = source, value = 0; ((width > 0) && (*source >= '0') && (*source <= '9')); source++, width--)
...
static int _fmtout(int handle, char *dest, const char *fmt, va_list arg)
{
    ...
    char fComma, fill;
    char *save_dest;
    char *ptr2, pbuffer[CMDBUFSIZ], ibuffer[32];    // 20100130 AB chanded from 2048 to CMDBUFSIZ
...
next_fmt:
            ...
            case 'q':   // 64-bit (quad) decimal
                ...
                ptr = lltoa( va_arg( arg, long long ), ibuffer, 10 );
#else
...
// convert integer to ASCII minimum 2 characters
void IntToAscii2(int nVal, char *pszBuf)
{
    sprintf( pszBuf, "%02u", nVal );
}
