...
void dm_report_field_set_value(struct dm_report_field *field, const void *value, const void *sortvalue)
{
	...
	field->sort_value = sortvalue ? sortvalue : value;
}
			 ...
			 uint32_t flags)
{
	...
	list_iterate_items(fp, struct field_properties, &rh->field_props) {
			      ...
			      struct field_properties *fp, void *object)
{
	...
	return (char *)ret + rh->fields[fp->field_num].offset;
}
...
static int _sort_rows(struct dm_report *rh)
{
	...
	list_iterate_items(row, struct row, &rh->rows)
		(*rows)[count++] = row;
