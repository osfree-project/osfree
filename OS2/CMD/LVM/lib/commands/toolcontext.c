...
static int _init_tag_configs(struct cmd_context *cmd)
{
        ...
        list_iterate_items(sl, struct str_list, &cmd->tags) {
...
static int _merge_config_files(struct cmd_context *cmd)
{
        ...
        list_iterate_items(cfl, struct config_tree_list, &cmd->config_files) {
...
static int _init_filters(struct cmd_context *cmd, unsigned load_persistent_cache)
{
        ...
        if (cache_dir || cache_file_prefix) {
                if (dm_snprintf(cache_file, sizeof(cache_file),
                    ...
                    cache_dir ? cache_dir : DEFAULT_CACHE_SUBDIR,
                    //cache_file_prefix ? : DEFAULT_CACHE_FILE_PREFIX) < 0) {
                    cache_file_prefix ? cache_file_prefix : DEFAULT_CACHE_FILE_PREFIX) < 0) {
        ...
        if (!(f4 = persistent_filter_create(f3, dev_cache ? dev_cache : cache_file))) {
        ...
        if (!cmd->is_static &&
            (cn = find_config_tree_node(cmd, "global/segment_libraries"))) {
                ...
                for (cv = cn->v; cv; cv = cv->next) {
                        ...
                        list_iterate_items(segtype2, struct segment_type, &cmd->segtypes) {
                                if ((segtype == segtype2) ||
