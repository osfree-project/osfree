...
uint32_t find_free_lvnum(struct logical_volume *lv)
{
        ...
        list_iterate_items(lvl, struct lv_list, &lv->vg->lvs) {
...
static int _log_parallel_areas(struct dm_pool *mem, struct list *parallel_areas)
{
        ...
        list_iterate_items(spvs, struct seg_pvs, parallel_areas) {
                //list_iterate_items(pvl, &spvs->pvs) {
                list_iterate_items(pvl, struct pv_list, &spvs->pvs) {
                                   ...
                                   struct logical_volume *log_lv)
{
        ...
        list_iterate_items(aa, struct alloced_area, &alloced_areas[0]) {
                        ...
                        void *data)
{
        ...
        area_len = (remaining_seg_len / area_multiple) ? area_len : 1;
                                ...
                                uint32_t *allocated, uint32_t needed)
{
        ...
        do {
                ...
                if (ah->parallel_areas) {
                        ...
                        list_iterate_items(spvs, struct seg_pvs, ah->parallel_areas) {
                ...
                list_iterate_items(pvm, struct pv_map, pvms) {
                        ...
                        if (alloc != ALLOC_ANYWHERE) {
                                ...
                                if (parallel_pvs)
                                        ...
                                        list_iterate_items(pvl, struct pv_list, parallel_pvs)
                                                if (pvm->pv == pvl->pv)
                                                        goto next_pv;
                        }
                        ...
                        list_iterate_items(pva, struct pv_area, &pvm->areas) {
                                if (contiguous) {
                                        if (prev_lvseg &&
