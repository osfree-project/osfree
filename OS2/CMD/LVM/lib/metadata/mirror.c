...
static void _move_lv_segments(struct logical_volume *lv_to, struct logical_volume *lv_from)
{
        ...
        list_iterate_items(seg, struct lv_segment, &lv_to->segments)
                seg->lv = lv_to;
...
static int _delete_lv(struct lv_segment *mirrored_seg, struct logical_volume *lv)
{
        ...
        if (!str_list_match_list(&mirrored_seg->lv->tags, &lv->tags)) {
                ...
                list_iterate_items(sl, struct str_list, &mirrored_seg->lv->tags)
                         ...
                         struct list *removable_pvs, int remove_log)
{
        ...
        if (removable_pvs) {
                for (s = 0; s < mirrored_seg->area_count; s++) {
                        ...
                        list_iterate_items(seg, struct lv_segment, &sub_lv->segments) {
                                for (s1 = 0; s1 < seg->area_count; s1++) {
                                        ...
                                        list_iterate_items(pvl, struct pv_list, removable_pvs) {
                          ...
                          struct list *lvs_changed)
{
        ...
        list_iterate_items(seg, struct lv_segment, &lv->segments) {
                for (s = 0; s < seg->area_count; s++) {
                        ...
                        list_iterate_items(per, struct pe_range, pvl->pe_ranges) {
                          ...
                          struct logical_volume *lv_mirr)
{
        ...
        list_iterate_items(lvl, struct lv_list, &vg->lvs) {
                ...
                list_iterate_items(seg, struct lv_segment, &lv1->segments) {
                        for (s = 0; s < seg->area_count; s++) {
...
const char *get_pvmove_pvname_from_lv_mirr(struct logical_volume *lv_mirr)
{
        ...
        list_iterate_items(seg, struct lv_segment, &lv_mirr->segments) {
                          ...
                          struct logical_volume *lv)
{
        ...
        list_iterate_items(lvl1, struct lv_list, &vg->lvs) {
                lv1 = lvl1->lv;
                if (lv1 == lv)
                        continue;
...
int fixup_imported_mirrors(struct volume_group *vg)
{
        ...
        list_iterate_items(lvl, struct lv_list, &vg->lvs) {
                ...
                list_iterate_items(seg, struct lv_segment, &lvl->lv->segments) {
                        if (seg->segtype !=
