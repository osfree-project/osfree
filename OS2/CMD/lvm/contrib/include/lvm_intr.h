...
{
CARDINAL32   Total_Available_Sectors;  /* The number of sectors on the disk which are not currently
                                          assigned to a partition. */
CARDINAL32   Largest_Free_Block_Of_Sectors;  /* The number of sectors in the largest contiguous block 
                                                 of available sectors.  */
BOOLEAN      Corrupt_Partition_Table;        /* If TRUE, then the partitioning information found on the 
                                                drive is incorrect! */
BOOLEAN      Unusable;                       /* If TRUE, the drive's MBR is not accessible and the drive 
                                                can not be partitioned. */
BOOLEAN      IO_Error;                       /* If TRUE, then the last I/O operation on this drive failed! */
BOOLEAN      Is_Big_Floppy;                  /* If TRUE, then the drive is a PRM formatted as a big floppy 
                                               (i.e. the old style removable media support). */
...
CARDINAL32   Boot_Limit;              /* The maximum number of sectors from this block of free space that */ 
				      /* can be used to create a bootable partition if you allocate from  */
				      /* the beginning of the block of free space.        */
...
BYTE         Active_Flag;       /* 80 = Partition is marked as being active. 0 = Partition is not active.  */
BYTE         OS_Flag;           /* This field is from the partition table.  It is known as the OS flag,    */
...
07 = The partition is a compatibility partition formatted for use with an installable filesystem, such as HPFS or JFS.
...
char         Volume_Name[VOLUME_NAME_SIZE];         /* If this partition is part of a volume, then this will be the name of    */
						    /* the volume that this partition is a part of.  If this record represents */ 
						    /* free space, then the Volume_Name will be "FREE SPACE xx", where xx is a */ 
						    /* unique numeric ID generated by LVM.DLL. Otherwise it will be an empty string.*/
...
*************************************/
BOOLEAN    New_Volume;  /* Set to FALSE if this volume existed before the LVM Engine was opened.   */  
                        /* Set to TRUE if this volume was created after the LVM Engine was opened. */
BYTE       Status;                                
/************************
0 = None.
1 = Bootable
...
/************************************* 
The following structure is used to specify an LVM Feature when creating a volume.  
Since LVM Features may be part of more than one LVM Class, the specific class 
to be used with the feature must also be specified.
**************************************/
...
/*                                    in which to store an error code*/
/*                                    should an error occur.         */
/*          int level  - debug level: 0-full, 1- partial             */
/*                                                                   */
...
/*                                                                   */
/*********************************************************************/
void _System Start_Logging( char * Filename, CARDINAL32 * Error_Code, int level );
