			     ...
			     struct dm_tree_node *child)
{
	...
	list_iterate_items(dlink, struct dm_tree_link, &parent->uses)
		if (dlink->node == child)
			return 1;
...
static void __unlink(struct list *list, struct dm_tree_node *node)
{
	struct dm_tree_link *dlink;

	list_iterate_items(dlink, struct dm_tree_link, list)
		if (dlink->node == node) {
			list_del(&dlink->list);
			break;
		}
			  ...
			  struct dm_tree_node *child)
{
	...
	__unlink(&parent->uses, child);
	__unlink(&child->used_by, parent);
}
			    ...
			    size_t paramsize, int *pos)
{
	...
	list_iterate_items(area, struct seg_area, &seg->areas) {
...
static int _load_node(struct dm_tree_node *dnode)
{
	...
	list_iterate_items(seg, struct load_segment, &dnode->props.segs)
