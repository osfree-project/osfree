...
void print_struct_r32_rlc_info(struct r32_rlc * rlc) {
	...
	int fixup_size = get_reloc_size_rlc(rlc);
...
int get_srcoff_cnt1_rlc(struct r32_rlc * rlc) {
	if((rlc->nr_stype & NRCHAIN)!= NRCHAIN) //0x20. Om den är satt så är det en DW annars DB.
		return rlc->r32_soff;
	unsigned char * ptr_uchar = (unsigned char *)&rlc->r32_soff;
	return *ptr_uchar /*>> 8*/;
}
...
int get_additive_rlc(struct r32_rlc * rlc) {
	...
	int additive_val=0;
	int additive_size = get_additive_size(rlc);
	unsigned long int uint_rlc = (unsigned long int)rlc;
	void *void_additive = (void*)(uint_rlc + additive_offs);
...
int get_srcoff12_rlc(struct r32_rlc * rlc, int idx) {
	...
	int srcoff12_val=0;
	int srcoff12_size = get_additive_size(rlc);
	unsigned long int uint_rlc = (unsigned long int)rlc;
	void *void_srcoff12 = (void*)(uint_rlc + srcoff12_offs + idx * 2);
