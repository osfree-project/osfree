...
size_t lx_fread_stream(void *ptr, size_t size, size_t nmemb, 
	...
	for(i=0; i<nmemb; i++) {
		...
		unsigned long int tmp_ptr = (unsigned int) ptr;
		tmp_ptr += size;
		ptr = (void *) tmp_ptr;
	}
	return i;
...
load_lx(FILE* fh, struct LX_module * lx_mod) {
	...
	int fstorlek = fseek(fh, 0, SEEK_END);
	fstorlek = fstorlek;
	fseek(fh, 0, SEEK_SET);
...
int load_lx_module_header(struct LX_module * lx_mod) {
	...
	if(lx_module_header_offset == 0) {
		...
		if((exe_sig[0] == 'L') && (exe_sig[1] == 'X')) {
			...
			struct e32_exe * lx_e32_exe = (struct e32_exe *) malloc(sizeof(struct e32_exe));
		...
		*/
		if((entry_ord_to_search >= entry_ord_index) 
		   ...
		   &&  (entry_table->b32_type == ENTRY32)) { /* 32-bit Entry.*/
				...
				unsigned long int i_cptr_ent_tbl = (unsigned long int)cptr_ent_tbl;
				int elements_in_bundle = entry_table->b32_cnt + entry_ord_index;
