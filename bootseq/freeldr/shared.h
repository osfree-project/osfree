...
/* Command-line buffer for Multiboot kernels and modules. This area
   ...
   that's no problem.  */
#define MB_CMDLINE_BUF          RAW_ADDR (0x24000)
...
#define tolower freeldr_tolower
...
_Packed struct linux_kernel_header /* originally without _Packed */
{
  char code1[0x0020];
...
}; /* Originally with } __attribute__ ((packed)); */
...
//typedef void (*entry_func) (long, long, long, long, long, long);
typedef unsigned long entry_func;
...
extern unsigned short far io_map[];
//extern unsigned short io_map[];

/* booting a multiboot executable */
//void multi_boot (int start, int mb_info);
// __attribute__ ((noreturn));
void multi_boot (unsigned long start, unsigned long mb_info);
...
//long __cdecl get_mmap_entry (struct mmar_desc *desc, long cont);
...
long get_vbe_controller_info (unsigned long controller);

/* Get VBE mode information.  */
long get_vbe_mode_info (int mode_number, unsigned long mode);
...
char far *skip_to (unsigned long after_equal, char far *cmdline);
...
long safe_parse_maxint (char far **str_ptr, long *myint_ptr);
...
unsigned long __cdecl freeldr_open (char *filename);
...
unsigned long __cdecl freeldr_read (unsigned long buf, unsigned long len);

/* Reposition a file offset.  */
//int grub_seek (int offset);
unsigned long __cdecl freeldr_seek (unsigned long offset);

/* Close a file.  */
//void grub_close (void);
void __cdecl freeldr_close (void);

/* Terminate the work with files */
void __cdecl freeldr_term (void);
...
kernel_t load_image (char far *kernel, char far *arg, kernel_t suggested_type,
                     unsigned long load_flags);

//int load_module (char *module, char *arg);

unsigned long load_module (char far *module, char far *arg);

void create_vbe_module(void *ctrl_info, long ctrl_info_len,
