...
int
memcheck (unsigned long addr, int len)
{
  // Physical address:
...
void far *
memset (void far *start, char c, unsigned short len)
{
  char far *p = start;
...
void far *
memmove (void far *_to, const void *_from, int _len)
{

   char far *from = MK_FP(SEG_DS, _from);
   char far *to = _to;

   if (memcheck ((unsigned long) _to, _len))
     {
        if( from == to ) {
            return( to );
        }
...
//void __cdecl putdec(unsigned long byte)
{
  unsigned char b1;
  int b[32];
  signed int nb;
...
unsigned long
grub_read (unsigned char far *buf, unsigned long len) // far char *!!!
{
  ...
  return   muRead((unsigned long) filepos,
                  (unsigned char *) buf,
                  (unsigned long) len);
}
...
unsigned long
grub_seek (unsigned long offset) // noch nicht fertig!
{
...
kernel_t
load_image (char *kernel, char far *arg, kernel_t suggested_type,
            unsigned long load_flags)

{
        int exec_type = 0, align_4k = 1;
        unsigned long len, i;
        ...
        /* presuming that MULTIBOOT_SEARCH is large enough to encompass an
                executable header */
        unsigned char far buffer[MULTIBOOT_SEARCH];
...
//      if (!(*muOpen) (farpfName,farpfSize))
        if (muOpen(kernel, &fSize))
        {
          ...
          printf("%s opened successfully, size: %lu\r\n", kernel, fSize);
        }
...
//                if (MULTIBOOT_FOUND ((int) (buffer + i), len - i))
                if (MULTIBOOT_FOUND ((unsigned long) (buffer + i), len - i))
                {
        ...
        if (((type == KERNEL_TYPE_MULTIBOOT && ! (flags & MULTIBOOT_AOUT_KLUDGE))
                        ...
                        || strcmp (pu.elf->e_ident + EI_BRAND, "FreeBSD") == 0 /* Ersetzen */
 ...
 //     mbi.boot_device = (current_drive << 24) | current_partition;
        ...
        if (grub_read ((char *) RAW_ADDR (cur_addr), text_len) == text_len)
                ...
                cur_addr += data_len;
                        ...
                        /*if (!errnum)
                {
                memset (RAW_ADDR (cur_addr), 0, bss_len);
                cur_addr += bss_len;
...
//int
unsigned long
load_module (char *module, char far *arg)
{
   unsigned long len;
...
//        mll[mbi.mods_count].cmdline = (int) arg;
        mll[mbi.mods_count].cmdline = (unsigned long) arg;
...
kernel_func (char far *arg, unsigned long flags)
{
  ...
  len = strlen ((char *) FP_OFF(arg));
  printf("kernel_func: strlen(%s) = %d\r\n", (char *) FP_OFF(arg), len);
  /* Reset MB_CMDLINE.  */
  mb_cmdline = (char far*) MB_CMDLINE_BUF;
  ...
  memmove(mb_cmdline, (char *) FP_OFF(arg), len + 1);
  kernel_type = load_image ((char *) FP_OFF(arg), mb_cmdline, suggested_type, load_flags);
  if (kernel_type == KERNEL_TYPE_NONE)
  {
...
{
  unsigned long len = strlen ((char *) FP_OFF(arg));
  ...
  switch (kernel_type)
    ...
    case KERNEL_TYPE_MULTIBOOT:
      ...
      memmove (mb_cmdline,(char *) arg, len + 1);
      if (! load_module ((char *) arg, mb_cmdline))
        return 1;
      mb_cmdline += len + 1;
      break;
...
//mbi.mem_upper = 16384;     // (temporarily)
  ...
  printf("OS2LDR::KernelLoader():hello!\r\n");
