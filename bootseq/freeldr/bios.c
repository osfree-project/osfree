...
/* These are defined in asm.S, and never be used elsewhere, so declare the
   prototypes here.  */
extern long __cdecl
biosdisk_int13_extensions (long ax, long drive, unsigned long dap);

extern long __cdecl
biosdisk_standard (long ah, long drive,
                            long coff, long hoff, long soff,
                            long nsec, long segment);

extern long __cdecl
check_int13_extensions (long drive);

extern long __cdecl
get_diskinfo_standard (long drive,
...
#if 0
extern long __cdecl
get_diskinfo_floppy (long drive,
                     unsigned long cylinders,
                     unsigned long heads,
                     unsigned long sectors);
#endif
...
long
biosdisk (long read, long drive, struct geometry far *geometry,
          long sector, long nsec, long segment)
{
  long err;

  if (geometry->flags & BIOSDISK_FLAG_LBA_EXTENSION)
    {
      _Packed struct disk_address_packet // Originally without _Packed
      {
      ...
      } dap; // Originaly with __attribute__ ((packed))
      ...
      err = biosdisk_int13_extensions ((read + 0x42) << 8, drive, PHYS_FROM_NP(current_seg, &dap));
...
/* #undef NO_INT13_FALLBACK */
    ...
    {
      long cylinder_offset, head_offset, sector_offset;
      long head;
...
static long
get_cdinfo (long drive, struct geometry far *geometry)
{
  long err;
  //int err;

  _Packed struct iso_spec_packet // Originally without _Packed
  {
    unsigned char size;
  ...
  } cdrp; // Originally with __attribute__ ((packed))

  printk("freeldr_memset() #2: clean out cdrp");
  freeldr_memset (MK_FP(current_seg, &cdrp), 0, sizeof (cdrp));
  //grub_memset (&cdrp, 0, sizeof (cdrp));
  cdrp.size = sizeof (cdrp) - sizeof (cdrp.dummy);
  err = biosdisk_int13_extensions (0x4B01, drive, PHYS_FROM_NP(current_seg, &cdrp));
  if (! err && cdrp.drive_no == drive)
    {
      if ((cdrp.media_type & 0x0F) == 0)
      ...
      else
        {
          ...
          geometry->cylinders
            = ((unsigned long) cdrp.cylinders
               + (((unsigned long) (cdrp.sectors & 0xC0)) << 2));
...
long
get_diskinfo (long drive, struct geometry far *geometry)
{
  //int err;
  long err;
  ...
  if (drive & 0x80)
    {
      ...
      if (version)
        {
          _Packed struct drive_parameters // Originally without _Packed
          {
            unsigned short size;
            ...
            unsigned char dummy[16];
          } drp; // Originally with __attribute__ ((packed))

          /* It is safe to clear out DRP.  */
          printk("freeldr_memset() #3: clean out drp");
          freeldr_memset (MK_FP(current_seg, &drp), 0, sizeof (drp));
          ...
          err = biosdisk_int13_extensions (0x4800, drive, PHYS_FROM_NP(current_seg, &drp));
          if (! err)
            {
              /* Set the LBA flag.  */
              geometry->flags = BIOSDISK_FLAG_LBA_EXTENSION;
      ...
      //                             &geometry->sectors);
      err = get_diskinfo_standard (drive,
                                   PHYS_FROM_NP(current_seg, &geometry->cylinders),
                                   PHYS_FROM_NP(current_seg, &geometry->heads),
                                   PHYS_FROM_NP(current_seg, &geometry->sectors));
      if (err)
        return err;
