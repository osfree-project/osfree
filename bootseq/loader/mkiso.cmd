...
cd %ROOT%bin
call mkboot.cmd boot\sectors\eltorito.bin boot\loader\preldr0.mdl boot\loader\fsd\iso9660.mdl boot\bootblk

rem @mkboot.cmd boot\sectors\eltorito.bin boot\loader\preldr_mini.mdl boot\loader\fsd\iso9660.mdl boot\bootblk
goto mkiso

cd ..\..\..
set dirs=cd cd\boot cd\boot\loader cd\boot\loader\fsd cd\boot\loader\term cd\l4 cd\pns cd\os3 cd\l4ka
...
set files=preldr0.mdl preldr0.rel preldr_mini.mdl preldr_mini.rel preldr.ini freeldr.mdl freeldr.rel boot.cfg linux.mdl linux.rel chain.mdl chain.rel disk.mdl disk.rel bootos2.mdl bootos2.rel bsd.mdl bsd.rel
...
for %i in (%dirs%) do (@if exist files\%i (if not exist cd\%i mkdir cd\%i) && copy files\%i\* cd\%i)
...
%mkisofs% -b boot/bootblk -c boot/bootcat.bin -no-emul-boot -boot-info-table -boot-load-size 4 -iso-level 3 -relaxed-filenames -r -J -l -D -N -U -allow-lowercase -no-iso-translate -v -x .DS_Store -publisher "osFree (www.osfree.org)" -o %imgdir1%/osfree.iso cd
rem %mkisofs% -b boot/bootblk -c boot/bootcat.bin -no-emul-boot -boot-info-table -boot-load-size 4 -iso-level 3 -relaxed-filenames -r -J -joliet-long -l -D -N -U -allow-lowercase -no-iso-translate -v -x .DS_Store -publisher "osFree (www.osfree.org)" -o %imgdir1%/osfree.iso cd
