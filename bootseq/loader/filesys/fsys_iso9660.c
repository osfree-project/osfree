...
{
  unsigned short sector_size_lg2 = log2((*pbuf_geom).sector_size);
  char     shift;
  ...
  byte_offset &= ((*pbuf_geom).sector_size - 1);

  //__asm__ volatile ("shl%L0 %1,%0"
  //              : "=r"(sector)
  //              : "Ic"((int8_t)(ISO_SECTOR_BITS - sector_size_lg2)),
  //              "0"(sector));

  shift = ((int8_t)(ISO_SECTOR_BITS - sector_size_lg2));
  ...
  for (sector = 16 ; sector < 32 ; sector++)
    {
      ...
      if (PRIMDESC->type.l == ISO_VD_PRIMARY
          && !(*pgrub_memcmp)((char const *)PRIMDESC->id, ISO_STANDARD_ID, sizeof(PRIMDESC->id)))
        {
          ISO_SUPER->vol_sector = sector;
          INODE->file_start = 0;
          *pfsmax = PRIMDESC->volume_space_size.l;
          return 1;
        }
    }
  ...
  do
    {
      ...
      for (pathlen = 0 ;
           dirname[pathlen]
             && !(*pgrub_isspace)(dirname[pathlen]) && dirname[pathlen] != '/' ;
           pathlen++)
        ;
      ...
      while (size > 0)
        {
          ...
          for (; idr->length.l > 0;
            ...
            {
              const char *name = (char const *)idr->name;
...
iso9660_read (char *buf, int len)
{
  ...
  ret = 0;
  blkoffset = *pfilepos & (ISO_SECTOR_SIZE - 1);
  sector = *pfilepos >> ISO_SECTOR_BITS;
  while (len > 0)
    {
