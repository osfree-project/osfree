...
next_extent (void)
{
        if (++jfs.xindex < jfs.xlastindex) {
                ...
                (*pdevread) (xtpage->header.next << jfs.bdlog, 0,
                         sizeof(xtpage_t), (char *)xtpage);
                jfs.xlastindex = xtpage->header.nextindex;
...
di_read (u32 inum, dinode_t *di)
{
        ...
        do {
                ...
                if (isinxt (key, offset, lengthXAD (xad))) {
                        (*pdevread) ((addressXAD (xad) + key - offset) << jfs.bdlog,
                                 3072 + xd*sizeof(pxd_t), sizeof(pxd_t), (char *)&pxd);
                        (*pdevread) (addressPXD (&pxd) << jfs.bdlog,
...
next_dentry (void)
{
        ...
        if (jfs.dttype == DTTYPE_INLINE) {
                ...
                if (jfs.sindex < jfs.slastindex) {
                        ...
                        (*pdevread) (dtpage->header.next << jfs.bdlog, 0,
...
first_dentry (void)
{
        ...
        if (dtr->header.flag & BT_LEAF) {
                ...
                for (;;) {
                        (*pdevread) (addressPXD (xd) << jfs.bdlog, 0,
                                 sizeof(dtpage_t), (char *)dtpage);
...
jfs_mount (void)
{
        ...
        if (*ppart_length < MINJFS >> SECTOR_BITS
            ...
            || !(*pdevread) ((AITBL_OFF >> SECTOR_BITS) + FILESYSTEM_I,
