...
extern int    (*pgrub_open)    (char *filename);
extern int    (*pgrub_read)    (char *buf, int len);
extern int    (*pgrub_seek)    (int offset);
extern int    (*pgrub_dir)     (char *dir);
extern void   (*pgrub_close)   (void);
extern void   (*pgrub_printf)  (const char *format,...);
...
char splashimage[64] = "/boot/loader/splas.xpm";
...
graphics_init (void)
{
    ...
    if (! read_image (splashimage))
    {
        ...
        (*pgrub_printf)("failed to read image\n");
        return 0;
    }
...
graphics_cls (void)
{
    ...
    /* plano 1 */
    MapMask(1);
    (*pgrub_memmove)(mem, s1, 38400);

    /* plano 2 */
    MapMask(2);
    (*pgrub_memmove)(mem, s2, 38400);

    /* plano 3 */
    MapMask(4);
    (*pgrub_memmove)(mem, s4, 38400);
...
read_image (char *s)
{
	...
	*perrnum = 0;
    ...
    /* read header */
    if (! (*pgrub_read)((char*)&buf, 10) || (*pgrub_memcmp)(buf, "/* XPM */\n", 10)) { //, 0xedde0d90)
        (*pgrub_close)();
        return 0;
    }
    ...
    while ((*pgrub_read)((char *)&c, 1) && (c == ' ' || c == '\t')) // , 0xedde0d90)
        ;
    ...
    base = 0;
    while ((*pgrub_read)((char *)&c, 1) && c != '"') // , 0xedde0d90)
        ;
    ...
    for (i = 0, idx = 1; i < colors; i++) {
        ...
        while ((*pgrub_read)((char *)&c, 1) && c != '"') // , 0xedde0d90)
            ;
        (*pgrub_read)((char *)&c, 1); // , 0xedde0d90)       /* char */
        base = c;
        (*pgrub_read)(buf, 4); // , 0xedde0d90)      /* \t c # */
    ...
    while (y < height) {
        while (1) {
            if (!(*pgrub_read)((char *)&c, 1)) { // , 0xedde0d90)
                (*pgrub_close)();
                return 0;
            }
            if (c == '"')
