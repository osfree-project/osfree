...
extern int winetest_ok( LONG condition, const char *msg, ... );
...
/* global flag on whether --log is chosen */
int isLogged = 0;
...
static DWORD winetest_lasterror;

#define GetProcessHeap() (HANDLE) -1

DWORD WINAPI GetLastError(void)
{
    return winetest_lasterror;
}

VOID WINAPI SetLastError(DWORD value)
{
    winetest_lasterror = value;
}

LPVOID WINAPI HeapAlloc(HANDLE hheap, DWORD flags, size_t size)
{
    return GlobalLock(GlobalAlloc(GPTR, size));
}
 ...
 */
/* condition was changed to 'LONG' so 32 bit comparison can be performed.*/
int winetest_ok( LONG condition, const char *msg, ... )
{
    va_list valist;
    tls_data* data=get_tls_data();
...
void main( int argc, char **argv )
{
	...
	if(argv[1])
	{
		/* option --log: redirect test result to a log file */	
		if (!strcmp(argv[1], "--log"))
    	{
			isLogged = 1;
			sprintf(log_name, "%s.log", winetest_testlist[0].name);	
			fp = freopen(log_name, "w", stdout);	
    	}
	}
